...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.error('Error decrypting Google Calendar tokens:', error);
    return res.status(500).json({ message: 'Failed to decrypt Google Calendar tokens' });
  }
  ...
  try {
    ...
    // @ts-ignore
    res.status(error.response?.status || 500).json({ message: 'Failed to fetch Google Calendar list' });
  }
}
